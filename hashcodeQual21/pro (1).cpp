...
unordered_map<string, int> vehAtInt;
...
int main() {
    ...
    for(int i = 0; i < I; i++) {
        ...
        for(int j = 0; j < S; j++) {
            int temp2 = (D / incomAtInt[i]) * ( (vehAtInt[streets[j].name]) / pow(V, 0.8) );
            if(streets[j].end == i && temp2 > 0 && temp2 <= D) {
                // res[k].push_back(make_pair(streets[j].name, D / (I*incom[i]) ));
                temp.push_back(make_pair(streets[j].name, temp2 ) );
            }
        }
        if(!temp.empty()) {
            resrep.push_back(i);
